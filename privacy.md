...
<div class="privacy-summary">
  <div><strong>不要求註冊</strong><span>本站本身沒有會員帳號，也不直接處理付款卡號。</span></div>
  <div><strong>偏好留在本機</strong><span>主題、播放器、搜尋與編輯紀錄等多數設定存在你的瀏覽器。</span></div>
  <div><strong>功能才會傳送</strong><span>角色查詢、AI 分析、留言等資料，只在你使用該功能時送往對應服務。</span></div>
</div>

## 這份政策適用在哪裡

本政策適用於 `holybear.tw` 及其下的文章、技術文件與網頁工具。連往其他網站後，將由該網站自己的隱私政策負責。

## 瀏覽本站時可能產生的資料

### 基本連線與廣告資訊

為了傳送網頁、維持安全、了解網站使用情況與顯示廣告，網站主機、CDN 及第三方服務可能處理 IP 位址、瀏覽器與裝置類型、來源頁面、訪問時間與頁面互動等一般技術資訊。

本站載入 Google AdSense 與 NEXON Open API Analytics。這些服務可能使用 Cookie 或類似技術來統計、防止濫用、投放及衡量廣告。你可在瀏覽器中阻擋或清除 Cookie，也可透過 [Google 廣告設定](https://myadcenter.google.com/) 管理個人化廣告。停用 Cookie 可能會影響部分功能，但不一定會消除所有廣告。

### 文章互動

文章的瀏覽次數與「推／噓」數量會以彙總形式儲存於 Google Firebase Cloud Firestore。瀏覽器另會在本機記住哪些文章已計數、你對文章的投票選擇，用來減少重複計數與顯示目前狀態。本站不為這些互動建立個人帳號。

文章留言由 Giscus 與 GitHub Discussions 提供。當你載入留言區或透過 GitHub 留言時，相關資料由 Giscus 與 GitHub 依其各自規則處理；公開留言也會顯示在對應的 GitHub Discussion。

## 儲存在你瀏覽器的資料

本站使用 `localStorage` 保留使用體驗所需的狀態，可能包括：

- 深淺色與背景主題、播放器音量與播放狀態。
- 文章互動狀態、通知顯示選擇與頁籤偏好。
- 文章編輯器的草稿、當前內容與歷史紀錄。
- 楓之谷工具的角色搜尋紀錄、收藏、計算器設定與功能導覽狀態。
- 你自行填入的 NEXON、Google Gemini、OpenAI 或自訂相容服務 API Key，以及自訂服務網址與模型名稱。

這些資料不會因為「儲存在本機」而自動上傳到 holybear.tw。但當你實際執行角色查詢或 AI 分析時，必要的 API Key、查詢條件與分析內容會由你的瀏覽器送往你選擇的服務。使用共用或不受信任的裝置時，不建議儲存私人 API Key。

你可透過各功能內的清除選項，或瀏覽器的「網站資料」設定刪除這些本機紀錄。
...
只有在你主動使用 AI 分析時，系統才會將建立分析所需的遊戲角色資料與提示內容，連同你填入的 API Key，直接從瀏覽器送往 Google Gemini、OpenAI 或你指定的自訂相容服務。請只使用你信任的 HTTPS 服務，並閱讀該服務的隱私與資料保留規則。

### 其他外部內容

部分頁面可能載入 GitHub 頭像、外部圖片、字型、天氣或其他內容。這類請求會讓對方服務收到一般連線資訊。部落格天氣卡片查詢的是固定的「新北市」天氣，不會讀取你的裝置定位。

## 我們如何使用與保護資料

上述資料只用於提供頁面與工具功能、保留你選擇的設定、統計內容互動、改善使用體驗、展示廣告，以及維持網站安全。本站不會出售你主動提供的個人資料。

本站採用 HTTPS 與受管理的第三方平台，並盡力限制網站自身保留的資料範圍。但任何網路傳輸或儲存方式都無法保證絕對安全。

## 你可以如何管理資料

- 在瀏覽器中清除 holybear.tw 的 Cookie 與網站資料。
- 在楓之谷工具的設定中移除 API Key，或刪除搜尋與收藏紀錄。
- 透過 Google 的廣告控制頁調整個人化廣告。
- 在 GitHub 上編輯或刪除你發佈的 Giscus 留言。
- 若想詢問與本站直接管理的資料，可使用下方聯絡方式。

## 第三方政策

第三方服務可能在不同國家或地區處理資料，並適用各自的使用條款與保留期間。你可參考：

- [Google 如何使用合作網站的資訊](https://policies.google.com/technologies/partner-sites?hl=zh-TW)
- [Firebase 的隱私與安全說明](https://firebase.google.com/support/privacy)
...
本站不是為 13 歲以下兒童設計的會員服務，也不會有意要求兒童提供個人資料。若你認為有不適合的資料被提供給本站，請與我聯絡。

## 政策更新與聯絡方式

網站功能或使用的第三方服務變動時，我可能更新這份政策，並在本頁標示新的更新日期。

如果你對本政策或資料處理方式有疑問，請寫信至 [holybear@holybear.tw](mailto:holybear@holybear.tw)。
