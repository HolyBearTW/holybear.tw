  ...
  <Theme.Layout>
    <template #doc-before>
      <div v-if="isBlogPost">
        ...
        <p>Debug: frontmatter.title = {{ frontmatter.value ? frontmatter.value.title : 'N/A' }}</p>
        <p>Debug: frontmatter.date = {{ frontmatter.value ? frontmatter.value.date : 'N/A' }}</p>
        <hr>
...
<style scoped>
/*
  這裡的樣式是針對在 #doc-before 插槽中渲染的元素。
  由於 VitePress 預設會從 Front Matter 的 `title` 生成一個 `<h1>` 標籤，
  這個 `<h1>` 會位於 `.vp-doc` 內部，我們需要隱藏它以避免重複標題。
*/
...
/* 這裡使用 :deep() 來穿透 scoped CSS，影響 .vp-doc 內部由 Markdown 生成的元素 */
:deep(.vp-doc p),
:deep(.vp-doc ul),
:deep(.vp-doc ol),
:deep(.vp-doc img),
