...
onMounted(async () => {
  ...
  const layers = [
    [back03.value, '/themes/core-tower/spine/back/BGA_03Building.skel', '/themes/core-tower/spine/back/core-tower.atlas', 'back03'],
    [back02.value, '/themes/core-tower/spine/back/BGA_02Building.skel', '/themes/core-tower/spine/back/core-tower.atlas', 'back02'],
    [back01.value, '/themes/core-tower/spine/back/BGA_01Building.skel', '/themes/core-tower/spine/back/core-tower.atlas', 'back01'],
    [foreground.value, '/themes/core-tower/spine/front/GA_01EV.skel', '/themes/core-tower/spine/front/core-tower-front.atlas', 'foreground']
  ] as const
  ...
  for (const [element, binaryUrl, atlasUrl, layer] of layers) {
    if (!element) continue
    ...
    const player = new SpinePlayer(element, {
      ...
      update(instance) {
        ...
        if (layer === 'foreground') {
          ...
          for (const name of floorBones) {
            const bone = instance.skeleton.findBone(name)
            if (bone) bone.y = bone.data.y + floorOffset
          }

          // On the tall phone crop, lower the canopy and its purple window
          // outlines together so the empty strip below the navigation is covered.
          const windowFrame = instance.skeleton.findBone('EV/Window')
      ...
      success(instance) {
        instance.speed = 1
        loadedLayers += 1
        if (loadedLayers === layers.length) spineReady.value = true
      },
