export const updateLogs = [  
  { date: '2026/08/26', content: '新增 BOSS 傷害計算機，可依角色目前輸出、實際擊殺時間與 BOSS 難度估算可攻略目標，並自動套用 API 回傳的角色等級、ARC／AUT 與對應增傷修正；同步整合至 AI 健檢、首次功能引導，補齊手機版及深淺色介面。能力雷達改為主屬、攻魔、攻魔%、Boss 總傷、爆傷與無視六軸，依台版角色樣本換算同職業等價主屬，實線與整體虛線皆可查看數值；另修正萌獸啟用判定，將攻魔%與終傷分開計算及標示，並加入每週自動更新台版雷達參考資料與網站部署。' },
