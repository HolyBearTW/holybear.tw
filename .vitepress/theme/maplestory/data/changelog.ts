export const updateLogs = [  
  { date: '2026/08/29', content: '改善角色搜尋體驗：搜尋框會記住上次輸入的角色名稱，收藏角色與搜尋紀錄會顯示頭像、伺服器、排名及戰力；未列入近期排名的角色會改從 Nexon 資料補上頭像與伺服器。輸入角色名稱時，近期戰力排名會同步顯示該角色的排名、暱稱、伺服器圖示與戰力；同時修正搜尋面板點擊後未收起的問題，並加強淺色模式下排名卡片與前三名外框的辨識度。' },
  { date: '2026/08/29', content: '全面改善裝備資訊視窗，讓顯示方式更貼近遊戲：武器新增「武器」、「單手／雙手」標籤，套用自由造型時會顯示目前外型並使用變更後的裝備圖示；新版靈魂武器可查看階段、等級、攻魔加成、靈魂烙印與可用技能，舊版靈魂寶珠也會以滿魂 1000 顯示攻魔加成、寶珠能力與技能，兩者統一使用火焰標記；補上永恆披風、鞋子、手套、光輝 Boss 裝備、米特拉的憤怒及全面控制核心的白金神奇剪刀可使用次數；修正能力數字意外換行、星力加成顏色與寶玉強化數值拆分顯示；並修復寶玉、機器人、圖騰、七日怪物公園看守者及古代石板複製品等道具說明的缺字與亂碼。' },
