export const updateLogs = [  
  { date: '2026/08/22', content: '全面強化戰力工具的淺色模式與 AI 健檢：修正成長檔案建立按鈕、備註、月光寵物標籤、AUT 區塊與聯盟攻擊隊預設按鈕的配色，保留 AUT 螢光陰影並降低淺色模式亮度，成長檔案建立流程也補上與 AI 健檢一致的動態效果；新增 Gemini 3.7 Flash，以及 OpenAI GPT-5.6 Sol、Terra、Luna 模型支援，Sol 另提供 Pro 與快速模式，並補齊 Gemini／OpenAI API Key 的初次設定、申請連結、錯誤提示與額度說明；重構 AI 健檢輸入，補入完整裝備、超級屬性、傳授技能、聯盟戰地／神器／冠軍、寵物、符文、套裝、V／HEXA 核心、HEXA 屬性、怪怪卡、武陵與七日成長等戰鬥資料，同時精簡提示與備援流程以縮短等待，保留並加長玩家喜愛的幽默專家點評；移除燦爛的凶星未開放提示，將普通／困難單人門檻更新為 5 億／15 億，並因應新版靈魂寶珠系統取消未裝備時的扣分與硬性改善要求；此外統一全站導航列的互動樣式，讓網站標題、一般連結、服務／佈景主題、語言與社群按鈕在滑鼠移入、鍵盤焦點及選單展開時都會正確變色，下拉選單標題的粗體字重也與其他項目一致；優化角色查詢完成後的畫面更新流程，優先顯示主要戰力資訊並延後建立龐大的詳細資料區塊，同時避免背景資料回傳時重複繪製整頁；進一步隔離近期戰力排名的畫面更新，延後建立近一年成長圖表、365 格日曆與大事記，減輕兩項資料同時出現時的短暫頓挫；首頁近期戰力排行也新增轉圈與柔光載入動畫，並預留卡片高度以降低內容載入時的版面跳動；AI 健檢新增 LV295 BOSS「尤比太」，單吃最低門檻設為 30 億戰力；同時加入無回應、串流停滯、429、空回應及伺服器錯誤的自動模型備援，Gemini 依序回退至 3.7 Flash、3.6 Flash、2.5 Flash，GPT 則回退至 5.6 Terra、Luna；尚未手動指定模型時也會依已填入的 API Key 智慧選擇，兩種 Key 皆有時優先採用 GPT-5.6 Terra。' },
