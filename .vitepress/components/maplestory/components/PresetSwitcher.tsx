...
const PresetSwitcher: React.FC<PresetSwitcherProps> = ({ 
  ...
  return (
    <div className={`maple-preset-switcher flex flex-col gap-2 mb-4 bg-slate-900/50 p-3 rounded-lg border border-slate-700/50 ${className}`}>
      <div className="flex justify-between items-center gap-1">
        
        {/* Left Side: Label and Badge/Extra */}
        <div className="flex items-center gap-2 min-w-0">
            {/* Label */}
            <span className="maple-preset-label text-sm font-bold text-slate-300 whitespace-nowrap">{label}切換</span>
        ...
        <div className="flex gap-1 shrink-0 ml-auto pl-1">
            {buttons.map((num) => (
              <button
                ...
                className={`
                  maple-preset-button w-8 h-8 rounded-md text-sm font-bold transition-all flex items-center justify-center relative flex-shrink-0
                  ${currentPreset === num 
                    ? 'is-current bg-indigo-600 text-white shadow-[0_0_10px_rgba(79,70,229,0.4)] border border-indigo-400'
              ...
              >
                ...
                */}
                {((activeNum === num) || (num === 0 && !activeNum && showBase)) && (
                  <span className="maple-preset-live-dot absolute -top-1 -right-1 w-2.5 h-2.5 bg-green-500 border-2 border-slate-900 rounded-full"></span>
                )}
              </button>
            ))}
        </div>
      ...
      {(currentPreset !== activeNum && !(currentPreset === 0 && !activeNum)) && (
        <div className="maple-preset-warning flex items-start gap-2 text-[11px] text-yellow-500/80 bg-yellow-900/10 p-2 rounded border border-yellow-900/20 leading-relaxed animate-in fade-in slide-in-from-top-1">
