  ...
  const prompt = `
    ...
    **【重要語言規範】**
    ...
    3. **特殊道具判定：** * **塔戒 (Seed Rings) / MX-131 / 黑翼胸章：** 此類裝備無法衝星與洗潛能（MX-131/黑翼為胸章），顯示「0星/無潛能」為正常現象。只要裝備清單中包含此類道具，即代表該玩家具備高階配裝觀念，請直接視為「加分項目」並給予正面評價。
       ...
       * **卓越強化 (Exceptional Enhancement)：** 若發現漆黑BOSS裝備 (Pitch Boss Set) 已經過「卓越強化/卓越鐵鎚」系統強化 (通常數值會比一般同星數裝備更高)，代表該玩家投入了巨額成本，**請務必給予極高評價**。
    ...
    **【⚠️ 絕對邏輯運算守則 (Strict Logic Gate)】**
    ...
    2.  **BOSS 攻略建議 (關鍵指標)：** 
        ...
        *   **完整列表強制要求：** 表格必須包含以下 **14 隻** BOSS，嚴禁遺漏或截斷：
             **史烏、戴米安、露希妲、威爾、戴斯克、頓凱爾、真‧希拉、守護天使綠水靈、黑魔法師、賽蓮、卡洛斯、最初的敵對者、燦爛的兇星 (標註尚未開放)、咖凌、林波**。
    ...
    4.  **點評：** 
        ...
        *   **自由發揮：** 請**不要**硬湊上述括號內的範例詞彙。請根據玩家的實際裝備狀況（例如：如果是新手就鼓勵他，如果是神裝就調侃他是不是橘子的親兒子），自然地融入 1~2 個台灣楓之谷社群的梗。
        *   **參考梗（僅供靈感，勿生硬堆砌）：** 廣播收點、尬廣跟上、穿成掉寶裝、輪迴燃燒、自由市場奸商、弓箭手村發呆、測謊機、斷線連連、橘子機率、22星炸裝、武器/副武/能源附加洗不到大雙或三排物攻、練等到暴斃...等。
        *   讓這段話成為整份報告最令人會心一笑的亮點，要像個真實的台灣資深玩家在對話！
