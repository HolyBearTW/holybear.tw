...
export const analyzeCharacter = async (data: DashboardData, apiKey: string, modelId: string = 'gemini-3.0-flash'): Promise<string> => {
  ...
  const prompt = `
    ...
    6. **評分標準 (需綜合考量 裝備/ARC/AUT/等級)：** 
       ...
       - **10分 (頂尖神人)：** 頂級永恆/25星/光輝BOSS裝備/命運武器解放，等級 285+ (卡爾西溫/塔拉哈特)，AUT 極高 (600~700+)。
    ...
    7. **高階 BOSS 屬性需求表 (ARC & AUT)：**
       **[ARC (秘法符文) 區域]**
       ...
       - **LV255+ 黑魔法師 (Black Mage):** ARC 1320 (困難: LV265/275; 極限: LV275/280)

       **[AUT (真實符文) 區域]**
       - **LV260 賽爾尼溫 (賽蓮):** 一階 150 / 二階 200
       - **LV265 阿爾克斯-卡羅泰 (卡洛斯):** 簡單 200 / 普通一階 250 / 普通二階 300 / 混沌 330 / 極限 440
       - **LV270 奧迪溫 (最初的敵對者):** 簡單 220 / 普通 320 / 困難 340 / 極限 460
       - **LV275 桃源境 (咖凌):** 簡單 230 / 普通 330 / 困難 350 / 極限 480
    ...
    2.  **BOSS 攻略建議 (關鍵指標)：** 請根據角色的「戰鬥力」、「無視防禦」、「BOSS傷害」與「ARC/AUT」，依序評估以下高階 BOSS 的攻略可能性。
        ...
        **【關鍵判斷標準】：**
        * **ARC/AUT:** 必須嚴格檢視是否達標。
        * **格蘭蒂斯 BOSS (賽蓮/最初的敵對者/卡洛斯/燦爛的凶星/咖凌/林波/巴德利斯):** 血量極高。即使 AUT 達標，若戰鬥力未達頂尖 (單吃通常需 5 億以上)，請預設為「組隊可打」。
