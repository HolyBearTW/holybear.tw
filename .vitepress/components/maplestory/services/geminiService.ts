...
export const analyzeCharacter = async (data: DashboardData, apiKey: string, modelId: string = 'gemini-2.5-flash'): Promise<string> => {
  ...
  const prompt = `
    ...
    2.  **BOSS 攻略建議 (關鍵指標)：** 
        ...
        **【關鍵判斷標準】：格蘭蒂斯地區 BOSS (賽蓮、卡洛斯、咖凌、林波、巴德利斯、最初的敵對者) 血量極高，單吃門檻極高。即使 AUT 達標，若戰鬥力未達頂尖水準 (例如：普通林波/咖凌單吃通常需 5 億以上戰鬥力)，請預設為「組隊可打」。請勿輕易給出「單吃」評價，除非機體數值極度溢出。**

        **【參考數據 (Reference Data)】**
        *   **Aut各地區BOSS需求:**
            *   **LV260賽爾尼溫:** 賽蓮一階:150 / 二階:200
            *   **LV265卡羅泰(卡洛斯):** 簡單:200 / 普通一階:250 / 二階:300 / 混沌:330 / 極限:440
            *   **LV270奧迪溫(最初的敵對者):** 簡單:220 / 普通:320 / 困難:340 / 極限:460
            *   **LV275桃源境(咖凌):** 簡單:230 / 普通:330 / 困難:350 / 極限:480
            *   **LV285卡爾西溫:** 林波:500
            *   **LV290塔拉哈特:** 巴德利斯:700
    ...
    * **重點：** 格蘭蒂斯地區的 BOSS 血量極高，請務必嚴格檢視「戰鬥力」與「AUT」是否雙雙達標，避免誤判為可單吃，尤其是終極模式幾乎一定要組隊才有可能攻略(大多格蘭蒂斯BOSS的終極模式需要至少2名以上玩家戰鬥力達15億以上)。
